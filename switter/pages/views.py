...
def main_page(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            PageState.objects.filter(user=user).update(page='main_page', profile=None, sub_window=None)
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'GET':
            ...
            if is_same_profile(username, logged_in_user):
                PageState.objects.filter(user=logged_in_user).update(page='profile_page', profile=username)
            else:
                PageState.objects.filter(user=logged_in_user).update(page='profile_page', profile=username, sub_window=None)
